<?php

namespace AppBundle\Entity;

use Doctrine\ORM\Mapping as ORM;

/**
 * Appartements
 *
 * @ORM\Table(name="appartements")
 * @ORM\Entity
 */
class Appartements
{
    /**
     * @var integer
     *
     * @ORM\Column(name="id", type="integer", nullable=false)
     * @ORM\Id
     * @ORM\GeneratedValue(strategy="IDENTITY")
     */
    private $id;

    /**
     * @var string
     *
     * @ORM\Column(name="nom", type="string", length=255, nullable=false)
     */
    private $nom;

    /**
     * @var string
     *
     * @ORM\Column(name="description", type="text", length=65535, nullable=false)
     */
    private $description;

    /**
     * @var string
     *
     * @ORM\Column(name="adresse1", type="string", length=255, nullable=false)
     */
    private $adresse1;

    /**
     * @var string
     *
     * @ORM\Column(name="adresse2", type="string", length=255, nullable=false)
     */
    private $adresse2;

    /**
     * @var integer
     *
     * @ORM\Column(name="cp", type="integer", nullable=false)
     */
    private $cp;

    /**
     * @var string
     *
     * @ORM\Column(name="ville", type="string", length=255, nullable=false)
     */
    private $ville;



    /**
     * Set nom
     *
     * @param string $nom
     * @return Appartements
     */
    public function setNom($nom)
    {
        $this->nom = $nom;

        return $this;
    }

    /**
     * Get nom
     *
     * @return string 
     */
    public function getNom()
    {
        return $this->nom;
    }

    /**
     * Set description
     *
     * @param string $description
     * @return Appartements
     */
    public function setDescription($description)
    {
        $this->description = $description;

        return $this;
    }

    /**
     * Get description
     *
     * @return string 
     */
    public function getDescription()
    {
        return $this->description;
    }

    /**
     * Set adresse1
     *
     * @param string $adresse1
     * @return Appartements
     */
    public function setAdresse1($adresse1)
    {
        $this->adresse1 = $adresse1;

        return $this;
    }

    /**
     * Get adresse1
     *
     * @return string 
     */
    public function getAdresse1()
    {
        return $this->adresse1;
    }

    /**
     * Set adresse2
     *
     * @param string $adresse2
     * @return Appartements
     */
    public function setAdresse2($adresse2)
    {
        $this->adresse2 = $adresse2;

        return $this;
    }

    /**
     * Get adresse2
     *
     * @return string 
     */
    public function getAdresse2()
    {
        return $this->adresse2;
    }

    /**
     * Set cp
     *
     * @param integer $cp
     * @return Appartements
     */
    public function setCp($cp)
    {
        $this->cp = $cp;

        return $this;
    }

    /**
     * Get cp
     *
     * @return integer 
     */
    public function getCp()
    {
        return $this->cp;
    }

    /**
     * Set ville
     *
     * @param string $ville
     * @return Appartements
     */
    public function setVille($ville)
    {
        $this->ville = $ville;

        return $this;
    }

    /**
     * Get ville
     *
     * @return string 
     */
    public function getVille()
    {
        return $this->ville;
    }

    /**
     * Get id
     *
     * @return integer 
     */
    public function getId()
    {
        return $this->id;
    }
    /**
     * @var \Doctrine\Common\Collections\Collection
     */
    private $idPhotos;

    /**
     * Constructor
     */
    public function __construct()
    {
        $this->idPhotos = new \Doctrine\Common\Collections\ArrayCollection();
    }

    /**
     * Add idPhotos
     *
     * @param \AppBundle\Entity\Photos $idPhotos
     * @return Appartements
     */
    public function addIdPhoto(\AppBundle\Entity\Photos $idPhotos)
    {
        $this->idPhotos[] = $idPhotos;

        return $this;
    }

    /**
     * Remove idPhotos
     *
     * @param \AppBundle\Entity\Photos $idPhotos
     */
    public function removeIdPhoto(\AppBundle\Entity\Photos $idPhotos)
    {
        $this->idPhotos->removeElement($idPhotos);
    }

    /**
     * Get idPhotos
     *
     * @return \Doctrine\Common\Collections\Collection 
     */
    public function getIdPhotos()
    {
        return $this->idPhotos;
    }
}
